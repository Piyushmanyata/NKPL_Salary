...
import { calendarDaysForMonth } from "../months";
 import {
   OFFICIAL_WAGE_DAYS,
   buildOfficialRow,
   normalizeCategory,
 ...
 describe("TICKET-14: targeted ticket regressions", () => {
  ...
  it("honours_manually_typed_extra_days", () => {
    const employee = refFrom({
       id: "t02",
       name: "Guard",
    ...
    expect(employee.extraDays).toBe(5);
    expect(employee.performanceBonus).toBeGreaterThan(0);
